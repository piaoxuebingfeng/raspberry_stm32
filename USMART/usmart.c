...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 2://查询指令
			...
			USART_printf(USART2,"\r\n");
			USART_printf(USART2,"-------------------------函数清单--------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)USART_printf(USART2,"%s\r\n",usmart_dev.funs[i].name);
			USART_printf(USART2,"\r\n");
			break;	 
		case 3://查询ID
			USART_ClearFlag(USART2, USART_FLAG_TC);
			USART_printf(USART2,"-------------------------函数 ID --------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)
			{
				usmart_get_fname((u8*)usmart_dev.funs[i].name,sfname,&pnum,&rval);//得到本地函数名 
				USART_printf(USART2,"%s id is:\r\n0X%08x\r\n",sfname,usmart_dev.funs[i].func); //显示ID
			}
			USART_printf(USART2,"\r\n");
			break;
		case 4://hex指令
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						  	//进制转换功能
				{
					USART_printf(USART2,"HEX:0X%X\r\n",res);	   	//转为16进制
				}else if(i!=4)return USMART_PARMERR;//参数错误.
				else 				   				//参数显示设定功能
				{
					USART_printf(USART2,"16进制参数显示!\r\n");
		...
		case 5://dec指令
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   	//进制转换功能
				{
					USART_printf(USART2,"DEC:%lu\r\n",res);	   	//转为10进制
				}else if(i!=4)return USMART_PARMERR;//参数错误.
				else 				   				//参数显示设定功能
				{
					USART_printf(USART2,"10进制参数显示!\r\n");
		...
		case 6://runtime指令,设置是否显示函数执行时间
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					if(USMART_ENTIMX_SCAN==0)USART_printf(USART2,"\r\nError! \r\nTo EN RunTime function,Please set USMART_ENTIMX_SCAN = 1 first!\r\n");//报错
					else
					{
						usmart_dev.runtimeflag=res;
						if(usmart_dev.runtimeflag)USART_printf(USART2,"Run Time Calculation ON\r\n");
						else USART_printf(USART2,"Run Time Calculation OFF\r\n"); 
					}
		...
		{  
			...
			if(sta)
			{
				switch(sta)
				{
					case USMART_FUNCERR:
						USART_ClearFlag(USART2, USART_FLAG_TC);
						USART_printf(USART2,"函数错误!\r\n");
						//printf("函数错误!\r\n");   			
						break;	
					case USMART_PARMERR:
						USART_ClearFlag(USART2, USART_FLAG_TC);
						USART_printf(USART2,"参数错误!\r\n");
						//printf("参数错误!\r\n");   			
						break;				
					case USMART_PARMOVER:
						USART_ClearFlag(USART2, USART_FLAG_TC);
						USART_printf(USART2,"参数太多!\r\n");
						//printf("参数太多!\r\n");   			
						break;		
					case USMART_NOFUNCFIND:
						USART_ClearFlag(USART2, USART_FLAG_TC);
						USART_printf(USART2,"未找到匹配的函数!\r\n");
						//printf("未找到匹配的函数!\r\n");   			
						break;		
				}
			}
